...
const kittyPrompts = {
  orangePetNames(array) {
    ...
    // ['Tiger', 'Snickers']
    const orangeCats = array.filter(cat => cat.color === 'orange');
  ...
  sortByAge(array) {
    ...
    const result = array.sort((cat1, cat2) => cat1.age - cat2.age).reverse();
    return result;
...
const classPrompts = {
  ...
  totalCapacities() {
    ...
    const result = classrooms.reduce((acc, room) => {
      if (!acc.feCapacity && room.program === "FE") {
        acc.feCapacity = room.capacity;
      } else if (acc.feCapacity && room.program === "FE") {
        acc.feCapacity += room.capacity;
      } else if (!acc.beCapacity && room.program === "BE") {
...
const turingPrompts = {
  studentsForEachInstructor() {
    ...
    const result = instructors.map(instructor => {
      return {name: instructor.name, studentCount: getStudentCount(instructor.module)};
    });
    return result;
